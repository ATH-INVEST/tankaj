...
function normalizeBrand(
  ...
  const text =
    `${rawBrand || ""} ${rawName || ""} ${rawOperator || ""}`.toUpperCase();
  ...
  return (
    rawBrand || rawOperator?.split(" ")[0] || rawName?.split(" ")[0] || null
  );
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const stationRows =
      limit > 0 ? stationRowsRaw.slice(0, limit) : stationRowsRaw;
    ...
    for (const row of priceRows) {
      ...
      if (fuelType === "PREMIUM_DIESEL" && (price < 1.3 || price > 3.8))
        continue;
