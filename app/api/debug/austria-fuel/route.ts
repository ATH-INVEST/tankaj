...
function mapFuelType(input: string | null): AustriaFuelType {
  ...
  if (value === 'bencin' || value === 'petrol' || value === '95' || value === 'sup') {
    return 'SUP'
  }
...
function getPrice(station: AustriaStation, fuel: AustriaFuelType): number | null {
...
export async function GET(req: NextRequest) {
  ...
  const normalized: NormalizedAustriaResult[] = raw
    .map((station): NormalizedAustriaResult | null => {
      ...
      const airDistanceKm = haversineKm(userLat, userLng, stationLat, stationLng)
